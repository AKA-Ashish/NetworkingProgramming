...
public class URI_Methods {
  ...
  public static void main(String[] args) {
    ...
    String uri_link2 =
        ...
        "https://user:password@subdomain.example.com/path/to/page?category=books&sort=price#section-2";  //double for comparision...
    ...
    try {
      ...
      System.out.println("URI Scheme : " + uri.getScheme());   //https or http

      System.out.println("URI Authority : " + uri.getAuthority());   //user.password@example.com

      System.out.println("URI Path : " + uri.getPath());   ///Search

      System.out.println("URI Query : " + uri.getQuery());  //after ? /q = dogs

      System.out.println("URI Fragments : " + uri.getFragment());  // after # results

      System.out.println(
          "parseServerAuthority() : " + uri.parseServerAuthority());

      System.out.println("normalize() : " + uri.normalize());  // Extra special character are trimed



      // RELATIVIZE() Method
      // Use : Relation Create to reach one URI from Another.
      // Here,baseURI ma imageURI ko image lai use garna paryo vane we can
      // relativize() create a relative URI that points to the image
      URI baseURI = new URI("https://www.example.com/foo/bar.html");
      URI imageURI = new URI("https://www.example.com/foo/images/logo.png");

      // imageURI lai chai baseURI(parent) sanga relate gareko & parentURI can use
      // components of childURI
      ...
      // true if the two URIs have the same scheme, authority, path, query, and
      // fragment components, and false otherwise.
      ...
      // True if isAbsolute->True && scheme-specific part does not begin with Slash
      // character '/'
